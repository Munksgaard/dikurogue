...
(defun process-ticks (state)
  (mapcar (lambda (x) (tick x state)) (cdr (assoc :entities state))))

(defun handle-input (state key)
  (sanitize-state
   (case key
     (:sdl-key-q ())
     (:sdl-key-h (progn (move-player :w state)
                        (process-ticks state)
                        (generate-screen state)))
     (:sdl-key-j (progn (move-player :s state)
                        (process-ticks state)
                        (generate-screen state)))
     (:sdl-key-k (progn (move-player :n state)
                        (process-ticks state)
                        (generate-screen state)))
     (:sdl-key-l (progn (move-player :e state)
                        (process-ticks state)
                        (generate-screen state)))
     (:sdl-key-period (progn (process-ticks state)
                             (generate-screen state)))
     (t state))))
...
(defparameter *default-state*
  (generate-screen
   (list (cons :max-x  79)
         ...
         (cons :entities (list (make-instance 'rat
                                              :symbol #\r
                                              :pos '(5 . 2)
                                              :hp 5
                                              :max-hp 5))))))
